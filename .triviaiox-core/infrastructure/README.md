...
Base layer of the TRIVIAIOX modular architecture. Contains tools, integrations, scripts, and PM adapters.
...
- `ComponentGenerator` - TRIVIAIOX component generation
- `BatchCreator` - Batch operations

### Validation
- `TriviaioxValidator` - TRIVIAIOX component validation
...
const {
  ...
  TemplateEngine,
  resolveTool
} = require('.triviaiox-core/infrastructure');

// Or import directly from scripts
const GitWrapper = require('.triviaiox-core/infrastructure/scripts/git-wrapper');
```

## Tool Resolution

```javascript
const { resolveTool } = require('.triviaiox-core/infrastructure');
...
```javascript
const { getPMAdapter, isPMToolConfigured } = require('.triviaiox-core/infrastructure');
