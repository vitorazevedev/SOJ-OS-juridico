 ...
 * for display in agent greetings across all 11 TRIVIAIOX agents.
 *
...
class ProjectStatusLoader {
  ...
  loadConfig() {
    try {
      const configPath = path.join(this.rootPath, '.triviaiox-core', 'core-config.yaml');
  ...
  _resolveCacheFilePath() {
    try {
      ...
      if (normalizedGitDir !== normalizedCommonDir) {
        ...
        return path.join(this.rootPath, '.triviaiox', `project-status-${worktreeHash}.yaml`);
      }
    } catch (error) {
      // Not a git repo or git not available - use default path
    }

    return path.join(this.rootPath, '.triviaiox', 'project-status.yaml');
  }
 ...
 * @property {string} path - Relative path to worktree (.triviaiox/worktrees/{story-id})
