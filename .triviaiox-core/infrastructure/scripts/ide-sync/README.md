...
Automatically synchronizes TRIVIAIOX agent definitions to IDE command files.

## Overview

IDE Sync keeps agent definitions in `.triviaiox-core/development/agents/` synchronized with IDE-specific command files in:

- `.claude/commands/TRIVIAIOX/agents/` (Claude Code)
- `.codex/agents/` (Codex CLI support files)
- `.gemini/rules/TRIVIAIOX/agents/` (Gemini CLI)
...
npm run sync:ide
# or
node .triviaiox-core/infrastructure/scripts/ide-sync/index.js sync
```
...
# or
node .triviaiox-core/infrastructure/scripts/ide-sync/index.js validate
```
...
# or
node .triviaiox-core/infrastructure/scripts/ide-sync/index.js validate --strict
```
...
Configure in `.triviaiox-core/core-config.yaml`:
...
ideSync:
  ...
  targets:
    ...
    # ... other IDEs
  redirects:
    triviaiox-developer: triviaiox-master
...
```markdown
# Agent Redirect: triviaiox-developer -> triviaiox-master

This agent has been renamed. Use `triviaiox-master` instead.
```

## File Structure

```
.triviaiox-core/infrastructure/scripts/ide-sync/
