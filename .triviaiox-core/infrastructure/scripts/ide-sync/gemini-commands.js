...
const FALLBACK_DESCRIPTION = 'Agente especializado TRIVIAIOX';
...
function commandSlugForAgent(agentId) {
  if (agentId.startsWith('triviaiox-')) {
    return agentId.replace(/^triviaiox-/, '');
  }
  return agentId;
}

function menuCommandName(agentId) {
  return `/triviaiox-${commandSlugForAgent(agentId)}`;
}
...
function buildAgentDescription(agent) {
  ...
  return `Ativar agente TRIVIAIOX ${agent.id}`;
}

function buildAgentCommandPrompt(agentId) {
  return [
    `Ative o agente ${agentId}:`,
    `1. Leia a definição completa em .gemini/rules/TRIVIAIOX/agents/${agentId}.md`,
    '2. Siga as activation-instructions do bloco YAML',
    `3. Renderize o greeting via: node .triviaiox-core/development/scripts/generate-greeting.js ${agentId}`,
...
function buildMenuPrompt(commandFiles) {
  const lines = [
    'Você está no launcher TRIVIAIOX para Gemini.',
    '',
...
function buildMenuCommandFile(commandFiles) {
  const content = [
    'description = "Menu rápido TRIVIAIOX (lista agentes e orienta qual ativar)"',
    'prompt = """',
