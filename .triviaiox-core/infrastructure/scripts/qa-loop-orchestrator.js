...
/**
 * TRIVIAIOX QA Loop Orchestrator
 *
...
const CONFIG = {
  ...
  // Workflow definition
  workflowPath: '.triviaiox-core/development/workflows/qa-loop.yaml',
  ...
  persistenceIndexPath: '.triviaiox/qa-loops-index.json', // Track all active loops
};
...
class QALoopOrchestrator {
  ...
  _loadMaxIterations() {
    ...
    const configPaths = [
      path.join(this.rootPath, '.triviaiox/config.yaml'),
      path.join(this.rootPath, '.triviaiox/config.yml'),
      path.join(this.rootPath, 'triviaiox.config.js'),
    ];
...
function printHelp() {
  console.log(`
📊 QA Loop Orchestrator - TRIVIAIOX QA Evolution (Story 6.5)
