 ...
 *
 * Detects installation mode for TRIVIAIOX projects with three-mode support:
 * - FRAMEWORK_DEV: Contributing to triviaiox-core itself
...
/**
 * Installation modes supported by TRIVIAIOX
 * @enum {string}
 */
...
const LegacyProjectType = {
  EXISTING_TRIVIAIOX: 'EXISTING_TRIVIAIOX',
  GREENFIELD: 'GREENFIELD',
  BROWNFIELD: 'BROWNFIELD',
  UNKNOWN: 'UNKNOWN',
};
...
const ModeDescriptions = {
  [InstallationMode.FRAMEWORK_DEV]: {
    ...
    hint: 'Developing triviaiox-core itself - uses framework standards, skips project setup',
    description: 'For TRIVIAIOX contributors working on the framework',
  },
  [InstallationMode.GREENFIELD]: {
    label: '🆕 New Project (Greenfield)',
    hint: 'Start a fresh project with TRIVIAIOX - generates project docs, config, and infrastructure',
    description: 'Empty directory setup with full scaffolding',
  },
  [InstallationMode.BROWNFIELD]: {
    label: '📂 Existing Project (Brownfield)',
    hint: 'Add TRIVIAIOX to existing project - analyzes current structure and adapts',
 ...
 * 1. FRAMEWORK_DEV - .triviaiox-core/ exists AND is triviaiox-core repo
...
function detectInstallationMode(targetDir = process.cwd()) {
  ...
  // Priority 1: Check for TRIVIAIOX framework development
  if (markers.hasTriviaioxCore && markers.isTriviaioxCoreRepo) {
    return {
      mode: InstallationMode.FRAMEWORK_DEV,
      legacyType: LegacyProjectType.EXISTING_TRIVIAIOX,
      confidence: 100,
      reason: 'Detected triviaiox-core repository with .triviaiox-core directory',
      markers,
    };
  }

  // Priority 2: Check for existing TRIVIAIOX installation (non-framework)
  if (markers.hasTriviaioxCore && !markers.isTriviaioxCoreRepo) {
    return {
      mode: InstallationMode.BROWNFIELD,
      legacyType: LegacyProjectType.EXISTING_TRIVIAIOX,
      confidence: 95,
      reason: 'TRIVIAIOX already installed in user project - treating as brownfield update',
      markers,
    };
  }
...
function collectMarkers(targetDir) {
  ...
  return {
    // TRIVIAIOX markers
    hasTriviaioxCore: fs.existsSync(path.join(targetDir, '.triviaiox-core')),
    isTriviaioxCoreRepo: isTriviaioxCoreRepository(targetDir),
...
/**
 * Checks if the target directory is the triviaiox-core repository itself
 *
 * @param {string} targetDir - Directory to check
 * @returns {boolean} True if this is the triviaiox-core repository
 */
function isTriviaioxCoreRepository(targetDir) {
  ...
  try {
    ...
    // Primary check: explicit triviaiox-core package names
    if (packageJson.name === '@triviaiox/core' || packageJson.name === 'triviaiox-core') {
      return true;
    }

    // Secondary check: workspaces pattern + triviaiox-specific marker file
    // This prevents false positives for generic monorepos
    const hasTriviaioxMarker = fs.existsSync(path.join(targetDir, '.triviaiox-core', 'infrastructure'));
    const hasWorkspaces =
      Array.isArray(packageJson.workspaces) && packageJson.workspaces.includes('packages/*');

    return hasWorkspaces && hasTriviaioxMarker;
  } catch (error) {
    // Log error for debugging but don't throw - return false for safety
    if (process.env.TRIVIAIOX_DEBUG) {
      console.warn(`[mode-detector] Error checking triviaiox-core repository: ${error.message}`);
    }
    return false;
  }
}
 ...
 * for EXISTING_TRIVIAIOX legacy type (both use EXISTING_TRIVIAIOX but with different modes).
 * For accurate mode detection, use the mode property from detectInstallationMode().
 *
 * @param {string} legacyType - Legacy project type (EXISTING_TRIVIAIOX, GREENFIELD, etc.)
 * @param {Object} [context] - Optional context for disambiguation
 * @param {boolean} [context.isTriviaioxCoreRepo] - True if this is the triviaiox-core repository
 * @returns {string} New installation mode
 */
function mapLegacyTypeToMode(legacyType, context = {}) {
  // EXISTING_TRIVIAIOX is ambiguous: it can be FRAMEWORK_DEV (triviaiox-core repo)
  // or BROWNFIELD (user project with TRIVIAIOX installed)
  if (legacyType === LegacyProjectType.EXISTING_TRIVIAIOX) {
    // Use context to disambiguate if provided
    if (context.isTriviaioxCoreRepo === true) {
      return InstallationMode.FRAMEWORK_DEV;
    }
    if (context.isTriviaioxCoreRepo === false) {
...
function validateModeSelection(selectedMode, detected) {
  ...
  if (selectedMode !== detected.mode) {
    ...
    if (selectedMode === InstallationMode.FRAMEWORK_DEV && !detected.markers.isTriviaioxCoreRepo) {
      result.warnings.push(
        'Selected framework-dev but this does not appear to be the triviaiox-core repository.',
      );
    }
