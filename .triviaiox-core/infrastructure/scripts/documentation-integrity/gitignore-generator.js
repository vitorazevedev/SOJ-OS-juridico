...
const GitignoreTemplates = {
  TRIVIAIOX_BASE: 'gitignore-triviaiox-base.tmpl',
...
function getTemplatesForStacks(techStacks) {
  const templates = [GitignoreTemplates.TRIVIAIOX_BASE];
...
function generateGitignore(markers, options = {}) {
  ...
  sections.push('# Generated by TRIVIAIOX Documentation Integrity System');
...
/**
 * Merges TRIVIAIOX ignores with existing .gitignore
 *
...
function mergeGitignore(existingContent, _options = {}) {
  // Check if TRIVIAIOX section already exists
  if (existingContent.includes('TRIVIAIOX Integration Section')) {
    console.log('TRIVIAIOX section already exists in .gitignore, skipping merge');
  ...
  try {
    ...
    mergeSection = `
# ========================================
# TRIVIAIOX Integration Section
# ========================================

# TRIVIAIOX Local Configuration
.triviaiox-core/local/
.triviaiox-core/*.local.yaml
.triviaiox-core/logs/
.triviaiox-core/cache/

# ========================================
# End of TRIVIAIOX Integration Section
# ========================================
`;
  }
...
/**
 * Checks if a .gitignore file has TRIVIAIOX integration
 *
 * @param {string} targetDir - Target directory
 * @returns {boolean} True if TRIVIAIOX section exists
 */
function hasTriviaioxIntegration(targetDir) {
  const gitignorePath = path.join(targetDir, '.gitignore');
  ...
  return content.includes('TRIVIAIOX Integration Section') || content.includes('.triviaiox-core/');
}
...
function parseGitignore(content) {
  ...
  let triviaioxStart = -1;
  let triviaioxEnd = -1;

  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];

    if (line.includes('TRIVIAIOX Integration Section')) {
      if (triviaioxStart === -1) {
        triviaioxStart = i;
      }
    }

    if (line.includes('End of TRIVIAIOX Integration Section')) {
      triviaioxEnd = i;
    }

    if (line.startsWith('#') && !line.includes('TRIVIAIOX')) {
  ...
  if (triviaioxStart !== -1) {
    sections.triviaioxSection = {
      start: triviaioxStart,
      end: triviaioxEnd !== -1 ? triviaioxEnd : lines.length - 1,
    };
  }
