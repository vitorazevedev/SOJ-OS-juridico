...
/**
 * TRIVIAIOX Plan Progress Tracker
 *
 ...
 *
 * Features:
 * - AC1: Located in `.triviaiox-core/infrastructure/scripts/`
 ...
 * - AC7: Integrates with `.triviaiox/dashboard/status.json`
 *
...
const CONFIG = {
  ...
  dashboardStatusPath: '.triviaiox/dashboard/status.json',
  // Legacy status path for backwards compatibility
  legacyStatusPath: '.triviaiox/status.json',
...
class PlanTracker {
  ...
  _findPlanPath() {
    const searchPaths = [
      ...
      // .triviaiox plans location
      path.join(this.rootPath, '.triviaiox/plans', this.storyId, CONFIG.implementationFile),
      // Epic-based location (e.g., triviaiox-core-ade/story-4.6)
   ...
   *
   * Updates `.triviaiox/dashboard/status.json` for dashboard integration.
   * Also updates legacy `.triviaiox/status.json` for backwards compatibility.
   *
...
async function main() {
  ...
  if (args.length < 1 || args.includes('--help') || args.includes('-h')) {
    console.log(`
\u{1F4CA} Plan Progress Tracker - TRIVIAIOX Execution Engine (Story 4.6)
