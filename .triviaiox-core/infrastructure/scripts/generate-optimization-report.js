...
// Layer: L2 (.triviaiox-core/infrastructure/scripts/)
// Purpose:
...
//
// Usage:
//   node .triviaiox-core/infrastructure/scripts/generate-optimization-report.js [options]
//
// Options:
...
const ANALYTICS_DIR = path.resolve(ROOT, '.triviaiox', 'analytics');
...
const TOOL_REGISTRY_FILE = path.resolve(ROOT, '.triviaiox-core', 'data', 'tool-registry.yaml');
...
function compareBaseline(baseline, usageData, registry) {
  ...
  for (const [wfName, wfData] of Object.entries(baselineWorkflows)) {
    ...
    const baselineOverheadPct = baseline.comparison?.triviaioxActual?.overheadPercentOfTypicalSession?.[wfName] || 0;
