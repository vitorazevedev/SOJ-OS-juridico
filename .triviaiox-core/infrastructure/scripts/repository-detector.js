 ...
 * @property {string} frameworkLocation - Path to TRIVIAIOX framework files
...
function detectRepositoryContext() {
  ...
  // Detect if we're in triviaiox-core repo itself
  const isFrameworkRepo =
    packageJson.name === '@triviaiox/fullstack' ||
    packageJson.name === 'triviaiox-core' ||
    remoteUrl.includes('triviaiox-core');

  // Load installation config if exists
  let installConfig = null;
  const configPath = path.join(cwd, '.triviaiox-installation-config.yaml');
  ...
  return {
    ...
    projectRoot: cwd,
    frameworkLocation: isFrameworkRepo ? cwd : path.join(cwd, 'node_modules/@triviaiox/fullstack'),
