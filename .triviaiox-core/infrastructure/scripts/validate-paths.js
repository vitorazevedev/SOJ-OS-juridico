...
function getDefaultOptions() {
  ...
  return {
    ...
    requiredFiles: [
      ...
      path.join(projectRoot, '.triviaiox-core', 'product', 'templates', 'ide-rules', 'codex-rules.md'),
    ],
    quiet: false,
    json: false,
  };
...
function listSkillFiles(skillsDir) {
  ...
  return fs.readdirSync(skillsDir, { withFileTypes: true })
    .filter(entry => entry.isDirectory() && entry.name.startsWith('triviaiox-'))
...
function validateSkillPathConventions(content, filePath) {
  const errors = [];
  if (!content.includes('.triviaiox-core/development/agents/')) {
    errors.push(`${filePath} missing canonical source path ".triviaiox-core/development/agents/"`);
  }
  if (!content.includes('.triviaiox-core/development/scripts/generate-greeting.js')) {
