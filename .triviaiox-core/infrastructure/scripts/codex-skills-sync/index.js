...
function getDefaultOptions() {
  ...
  const envLocalDir = process.env.TRIVIAIOX_CODEX_LOCAL_SKILLS_DIR;
  const envGlobalDir = process.env.TRIVIAIOX_CODEX_GLOBAL_SKILLS_DIR;
  return {
    projectRoot,
    sourceDir: path.join(projectRoot, '.triviaiox-core', 'development', 'agents'),
...
function getSkillId(agentId) {
  ...
  if (id.startsWith('triviaiox-')) return id;
  return `triviaiox-${id}`;
}
...
function buildSkillContent(agentData) {
  ...
  const title = agent.title || 'TRIVIAIOX Agent';
...
# TRIVIAIOX ${title} Activator

## When To Use
${whenToUse}

## Activation Protocol
1. Load \`.triviaiox-core/development/agents/${agentData.filename}\` as source of truth (fallback: \`.codex/agents/${agentData.filename}\`).
2. Adopt this agent persona and command system.
3. Generate greeting via \`node .triviaiox-core/development/scripts/generate-greeting.js ${agentData.id}\` and show it first.
...
## Non-Negotiables
- Follow \`.triviaiox-core/constitution.md\`.
