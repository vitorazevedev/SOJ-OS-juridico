/**
 * Component Generator for TriviaGrowth Triviaiox
...
class ComponentGenerator {
  constructor(options = {}) {
    ...
    this.templatesPath = path.join(this.rootPath, 'triviaiox-core', 'templates');
    this.outputPaths = {
      agent: path.join(this.rootPath, 'triviaiox-core', 'agents'),
      task: path.join(this.rootPath, 'triviaiox-core', 'tasks'),
      workflow: path.join(this.rootPath, 'triviaiox-core', 'workflows'),
    };
  ...
  processAnswersToVariables(componentType, answers) {
    ...
    switch (componentType) {
      ...
      case 'task':
        ...
        // Notes
        variables.EACH_NOTES = ['Generated using TriviaGrowth Triviaiox template system'];
        break;
      ...
      case 'workflow':
        ...
        // Metadata
        variables.AUTHOR = process.env.USER || 'TriviaGrowth Triviaiox';
  ...
  async createAgentCommandTasks(variables, options) {
    ...
    for (const command of variables.COMMANDS) {
      const taskId = this.commandToTaskId(command.COMMAND_NAME);
      const taskPath = path.join(this.rootPath, 'triviaiox-core', 'tasks', `${taskId}.md`);
