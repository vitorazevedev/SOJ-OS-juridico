...
function getDefaultOptions() {
  ...
  return {
    ...
    agentsDir: path.join(projectRoot, '.gemini', 'rules', 'TRIVIAIOX', 'agents'),
    commandsDir: path.join(projectRoot, '.gemini', 'commands'),
    extensionDir: path.join(projectRoot, 'packages', 'gemini-triviaiox-extension'),
    sourceAgentsDir: path.join(projectRoot, '.triviaiox-core', 'development', 'agents'),
    quiet: false,
    json: false,
  };
}
...
function validateGeminiIntegration(options = {}) {
  ...
  const resolved = {
    ...
    agentsDir: options.agentsDir || path.join(projectRoot, '.gemini', 'rules', 'TRIVIAIOX', 'agents'),
    commandsDir: options.commandsDir || path.join(projectRoot, '.gemini', 'commands'),
    extensionDir: options.extensionDir || path.join(projectRoot, 'packages', 'gemini-triviaiox-extension'),
    sourceAgentsDir: options.sourceAgentsDir || path.join(projectRoot, '.triviaiox-core', 'development', 'agents'),
  };
  const errors = [];
  const warnings = [];
  ...
  if (!commandFiles.includes('triviaiox-menu.toml')) {
    errors.push(`Missing Gemini command file: ${path.relative(resolved.projectRoot, path.join(resolved.commandsDir, 'triviaiox-menu.toml'))}`);
  }
  ...
  const requiredExtensionFiles = [
    'extension.json',
    'README.md',
    path.join('commands', 'triviaiox-status.js'),
    path.join('commands', 'triviaiox-agents.js'),
    path.join('commands', 'triviaiox-validate.js'),
