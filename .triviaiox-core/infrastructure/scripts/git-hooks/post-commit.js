...
/**
 * Git Post-Commit Hook - TRIVIAIOX ProjectStatusLoader Cache Invalidation
 *
 ...
 *     node .triviaiox-core/infrastructure/scripts/git-hooks/post-commit.js
 */
 ...
 * looking for .triviaiox-core directory.
 *
...
function findProjectRoot() {
  ...
  // Walk up looking for .triviaiox-core
  for (let i = 0; i < 10; i++) {
    if (fs.existsSync(path.join(dir, '.triviaiox-core'))) {
      return dir;
    }
...
/**
 * Clear all project-status cache files in the .triviaiox directory.
 * Handles both standard cache and worktree-specific cache files.
 */
function clearProjectStatusCache() {
  const projectRoot = findProjectRoot();
  const triviaioxDir = path.join(projectRoot, '.triviaiox');

  if (!fs.existsSync(triviaioxDir)) {
    return; // No .triviaiox directory - nothing to clear
  }

  try {
    const files = fs.readdirSync(triviaioxDir);

    for (const file of files) {
      // Match project-status.yaml and project-status-{hash}.yaml
      if (file.startsWith('project-status') && file.endsWith('.yaml')) {
        const filePath = path.join(triviaioxDir, file);
        try {
