...
const REFERENCE_PATTERNS = [
  // Markdown code refs to .triviaiox-core
  { pattern: /`\.triviaiox-core\/[^`]+`/g, type: 'code-ref' },
  // Task references
...
const ASSET_DIRS = {
  task: '.triviaiox-core/development/tasks',
  template: '.triviaiox-core/product/templates',
  checklist: '.triviaiox-core/product/checklists',
  script: '.triviaiox-core/infrastructure/scripts',
  data: '.triviaiox-core/development/data',
  schema: '.triviaiox-core/schemas',
  agent: '.triviaiox-core/development/agents',
};
...
async function findFilesToAnalyze(rootPath) {
  const files = [];
  const dirsToScan = [
    { dir: '.triviaiox-core/development/agents', type: 'agent' },
    { dir: '.triviaiox-core/development/tasks', type: 'task' },
    { dir: '.triviaiox-core/product/templates', type: 'template' },
    { dir: '.triviaiox-core/product/checklists', type: 'checklist' },
  ];
...
function resolveReference(ref, rootPath) {
  ...
  if (ref.reference.startsWith('.triviaiox-core/')) {
...
function formatConsoleOutput(report, verbose = false) {
  ...
  lines.push('');
  lines.push('═══════════════════════════════════════════════════════════');
  lines.push('  TRIVIAIOX Path Analysis Report');
...
Options:
  ...
  while (rootPath !== '/') {
    try {
      await fs.access(path.join(rootPath, '.triviaiox-core'));
      break;
    } catch {
  ...
  if (rootPath === '/') {
    console.error('Error: Could not find .triviaiox-core directory. Run from project root.');
    process.exit(1);
  }
