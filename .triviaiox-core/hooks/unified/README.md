...
**Module:** `.triviaiox-core/hooks/unified`
...
Hook Runners (runners/*.js)
         ↓
Application Logic (triviaiox-core, triviaiox-pro)
```
...
- **`precompact-runner.js`** (Story MIS-3)
  ...
  - Open Core architecture (delegates to triviaiox-pro)
...
pro/memory/session-digest/extractor.js
         ↓
.triviaiox/session-digests/{session-id}-{timestamp}.yaml
```
...
- If triviaiox-pro not available: no-op (log and return)
...
```javascript
// .triviaiox-core/hooks/unified/runners/my-runner.js
...
async function main() {
  ...
  const { onMyEvent } = require('../../.triviaiox-core/hooks/unified/runners/my-runner');
...
if (!isProAvailable()) {
  console.log('[Hook] triviaiox-pro not available, skipping');
  return;
}
...
- **Story PRO-5:** triviaiox-pro Repository Bootstrap (pro-detector pattern)

---

*Unified Hooks System - TRIVIAIOX Core*
