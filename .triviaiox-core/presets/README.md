...
TRIVIAIOX Presets are pre-configured bundles that provide ready-to-use configurations for specific use cases, project types, or team structures. Presets simplify the initial setup process and ensure consistent configuration across projects.
...
| ------------ | ----------------------- | ------------------ | ----------------- |
| `minimal`    | Bare minimum TRIVIAIOX setup | dev                | Quick prototyping |
...
\`\`\`bash
triviaiox install --preset my-preset
\`\`\`
```
...
triviaiox install --preset standard

# Install with multiple presets (layered)
triviaiox install --preset standard --preset frontend

# Install custom preset from directory
triviaiox install --preset ./presets/my-preset
```
...
triviaiox presets list

# List with details
triviaiox presets list --verbose
```
...
triviaiox presets show standard

# Compare two presets
triviaiox presets diff minimal standard
```
...
triviaiox presets export --name my-project-preset --output ./presets/

# Export with description
triviaiox presets export --name my-preset --description "Project template" --output ./presets/
```
...
```bash
triviaiox install --preset standard --preset frontend --preset small-team
```
...
2. **Review before install** - Use `triviaiox presets show` to understand what you're getting
3. **Export before modifying** - Save your config as a preset before major changes
4. **Keep presets updated** - Check for preset updates when upgrading TRIVIAIOX
...
_Last Updated: 2026-01-28 | TRIVIAIOX Framework Team_
