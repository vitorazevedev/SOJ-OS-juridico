...
$allTasks = Get-ChildItem .triviaiox-core/tasks/*.md | Where-Object { $_.Name -notlike '*v1-backup*' } | ForEach-Object { $_.Name }
...
foreach ($task in $allTasks) {
  $result = node .triviaiox-core/scripts/validate-task-v2.js ".triviaiox-core/tasks/$task" 2>&1
...
foreach ($task in $nonCompliantTasks) {
  ...
  node .triviaiox-core/scripts/migrate-task-to-v2.js ".triviaiox-core/tasks/$task" 2>&1 | Out-Null
