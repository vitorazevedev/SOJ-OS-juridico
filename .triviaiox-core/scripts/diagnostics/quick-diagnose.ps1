# TRIVIAIOX-Core Quick Diagnostic - Run this in PowerShell
# Usage: Copy and paste this entire script into PowerShell on the target machine

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "   TRIVIAIOX-Core Quick Diagnostic" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$registryTest = & npm view triviaiox-core version 2>$null
if ($registryTest) {
    Write-Host "  triviaiox-core v$registryTest available" -ForegroundColor Green
} else {
    Write-Host "  Cannot access npm registry!" -ForegroundColor Red
    Write-Host "  Check firewall/proxy settings" -ForegroundColor Yellow
}

# Test npx triviaiox-core
Write-Host ""
Write-Host "Testing: npx triviaiox-core@latest --version" -ForegroundColor Yellow
Write-Host "(This may take a moment...)" -ForegroundColor Gray
try {
    $result = & npx triviaiox-core@latest --version 2>&1
...
Write-Host "  npx triviaiox-core@latest" -ForegroundColor White
Write-Host ""
