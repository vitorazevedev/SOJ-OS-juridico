# Diagnóstico de problema com npx triviaiox-core
# Execute este script no PC com problema

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "   Diagnóstico NPX triviaiox-core" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "[4] Baixando triviaiox-core manualmente..." -ForegroundColor Yellow
$tempDir = "$env:TEMP\triviaiox-test-$(Get-Random)"
New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
Set-Location $tempDir

Write-Host "    Diretório: $tempDir"
npm pack triviaiox-core@2.2.0 2>&1 | Out-Null

if (Test-Path "triviaiox-core-2.2.0.tgz") {
    Write-Host "    Download: OK" -ForegroundColor Green

    # Extrair
    tar -xzf triviaiox-core-2.2.0.tgz 2>&1 | Out-Null

    if (Test-Path "package/bin/triviaiox.js") {
        Write-Host "    Extração: OK" -ForegroundColor Green

        # Tentar executar
        Write-Host ""
        Write-Host "[5] Executando bin/triviaiox.js --version:" -ForegroundColor Yellow
        try {
            $result = node package/bin/triviaiox.js --version 2>&1
        ...
        Write-Host "    Executando: node package/bin/triviaiox.js --help"
        node package/bin/triviaiox.js --help 2>&1

    } else {
        Write-Host "    Extração: FALHOU - bin/triviaiox.js não encontrado" -ForegroundColor Red
...
Write-Host "4. Tente novamente: npx triviaiox-core@latest"
Write-Host ""
