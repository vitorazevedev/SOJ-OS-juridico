#!/bin/bash
# TRIVIAIOX Framework Update - v5.2 (Optimized)
#
# LOGIC:
...
#
# Usage: bash .triviaiox-core/scripts/update-triviaiox.sh

set -e

echo "⚡ TRIVIAIOX Update v5.2"
echo ""
...
# Validate: clean working tree for .triviaiox-core
if [ -n "$(git status --porcelain .triviaiox-core/)" ]; then
  echo "❌ Commit .triviaiox-core changes first:"
  git status --short .triviaiox-core/
  echo ""
  echo "Run: git add .triviaiox-core && git commit -m 'your message'"
  exit 1
fi
...
git clone --depth 1 --filter=blob:none --sparse \
  https://github.com/Trivia-Growth/Triviaiox.git \
...
git sparse-checkout set .triviaiox-core
cd - > /dev/null

echo "✅ Fetched upstream"
echo ""

# Build file lists (relative paths without .triviaiox-core/ prefix)
echo "📋 Scanning files..."
find .triviaiox-core -type f | sed 's|^\.triviaiox-core/||' | sort > "$TEMP_DIR/local-files.txt"
find "$TEMP_DIR/upstream/.triviaiox-core" -type f | sed "s|^$TEMP_DIR/upstream/\.triviaiox-core/||" | sort > "$TEMP_DIR/upstream-files.txt"

# Get git-tracked files (for DELETE detection) - ONE call instead of N calls
git ls-files .triviaiox-core | sed 's|^\.triviaiox-core/||' | sort > "$TEMP_DIR/tracked-files.txt"
...
while IFS= read -r rel_path; do
  if ! cmp -s ".triviaiox-core/$rel_path" "$TEMP_DIR/upstream/.triviaiox-core/$rel_path"; then
...
while IFS= read -r rel_path; do
  ...
  cp -a ".triviaiox-core/$rel_path" "$TEMP_DIR/local-only/$rel_path"
...
if [ -s "$REPORT_DELETED" ]; then
  while IFS= read -r rel_path; do
    rm -f ".triviaiox-core/$rel_path"
  done < "$REPORT_DELETED"
fi

# Copy all upstream files (creates new + overwrites existing)
rsync -a "$TEMP_DIR/upstream/.triviaiox-core/" ".triviaiox-core/"

# Restore local-only files
if [ -d "$TEMP_DIR/local-only" ] && [ "$(ls -A "$TEMP_DIR/local-only" 2>/dev/null)" ]; then
  rsync -a "$TEMP_DIR/local-only/" ".triviaiox-core/"
fi

# Clean empty directories
find .triviaiox-core -type d -empty -delete 2>/dev/null || true
...
echo ""
echo "Choose:"
echo "  ✅ Apply:  git add .triviaiox-core && git commit -m 'chore: sync TRIVIAIOX framework'"
echo "  ❌ Cancel: git checkout -- .triviaiox-core/"
echo ""
