#!/bin/bash
#
# TRIVIAIOX Framework Documentation Migration Script
#
# Purpose: Migrate framework docs from triviaiox-fullstack to triviaiox-core (REPO 1)
...
#   --target-repo    Path to triviaiox-core repository (default: ../triviaiox-core)
...
echo ""
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  TRIVIAIOX Framework Documentation Migration Script             ║${NC}"
...
if [ ! -d "$SOURCE_DOCS" ]; then
  ...
  echo "  Please run this script from the triviaiox-fullstack root directory"
  exit 1
fi
...
if [ ! -d "$TARGET_REPO" ]; then
  ...
  echo "  Please clone triviaiox-core first or specify correct path with --target-repo"
  exit 1
fi
...
**Source:** triviaiox-fullstack/docs/framework/
**Target:** triviaiox-core/docs/framework/
**Story:** 6.1.2.6 - Framework Configuration System

## Migration Status

✅ Framework documentation successfully migrated from triviaiox-fullstack to triviaiox-core (REPO 1)

## Files Migrated

- coding-standards.md - Official TRIVIAIOX coding standards
...
- triviaiox-fullstack maintains copies in `docs/framework/` (will be removed Q3 2026)
- triviaiox-fullstack maintains legacy copies in `docs/architecture/` (will be removed Q3 2026)
...
```bash
# Restore from triviaiox-fullstack
cd triviaiox-fullstack
...
if [ "$DRY_RUN" = false ] && [ -d "$TARGET_REPO/.git" ]; then
  ...
  echo "    git commit -m \"docs: migrate framework documentation from triviaiox-fullstack\""
  echo ""
  echo "    Story: 6.1.2.6 - Framework Configuration System"
  echo "    Migrated: coding-standards.md, tech-stack.md, source-tree.md"
  echo "    Source: triviaiox-fullstack/docs/framework/"
  echo ""
...
echo "    4. Commit changes to triviaiox-core repository"
echo "    5. Update triviaiox-fullstack deprecation warnings (Q3 2026)"
echo ""
