#!/bin/bash
# ============================================
# TRIVIAIOX PM Orchestration Script (pm.sh)
...
#   --output    - Custom output file path (default: /tmp/triviaiox-output-{timestamp}.md)
#
# Environment Variables:
#   TRIVIAIOX_OUTPUT_DIR   - Directory for output files (default: /tmp)
#   TRIVIAIOX_DEBUG        - Enable debug logging (default: false)
#   TRIVIAIOX_TIMEOUT      - Timeout in seconds (default: 300)
...
# Configuration
OUTPUT_DIR="${TRIVIAIOX_OUTPUT_DIR:-/tmp}"
DEBUG="${TRIVIAIOX_DEBUG:-false}"
TIMEOUT="${TRIVIAIOX_TIMEOUT:-300}"
CLAUDE_CMD="${CLAUDE_CMD:-claude}"
INLINE_MODE="${TRIVIAIOX_INLINE_MODE:-false}"
...
show_help() {
  cat << EOF
TRIVIAIOX Multi-Modal Orchestration Script v${VERSION}
...
Environment Variables:
  TRIVIAIOX_OUTPUT_DIR    Output directory (default: /tmp)
  TRIVIAIOX_DEBUG         Enable debug mode (default: false)
  TRIVIAIOX_TIMEOUT       Timeout in seconds (default: 300)
  TRIVIAIOX_INLINE_MODE   Run without a visual terminal (default: false)
  CLAUDE_CMD         Claude CLI command (default: claude)
...
setup_paths() {
  ...
  if [[ -n "$CUSTOM_OUTPUT" ]]; then
    ...
    OUTPUT_FILE="${OUTPUT_DIR}/triviaiox-output-${timestamp}.md"
  fi

  LOCK_FILE="${OUTPUT_DIR}/triviaiox-lock-${timestamp}.lock"
...
spawn_inline() {
  ...
  # Build the command
  local output=""
  output+="=== TRIVIAIOX Agent Session (Inline) ===$(printf '\n')"
...
spawn_terminal() {
  ...
  local full_cmd="echo '=== TRIVIAIOX Agent Session ===' && "
...
main() {
  ...
  log_info "TRIVIAIOX Multi-Modal Orchestration Script v${VERSION}"
