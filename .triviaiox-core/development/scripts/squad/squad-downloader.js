 ...
 *
 * Downloads squads from the triviaiox-squads GitHub repository.
...
/**
 * Default registry URL for triviaiox-squads
 * @constant {string}
 */
const REGISTRY_URL =
  'https://raw.githubusercontent.com/Trivia-Growth/triviaiox-squads/main/registry.json';

/**
 * GitHub API base URL for triviaiox-squads contents
 * @constant {string}
 */
const GITHUB_API_BASE =
  'https://api.github.com/repos/Trivia-Growth/triviaiox-squads/contents/packages';
...
/**
 * Squad Downloader class for downloading squads from triviaiox-squads repository
 */
class SquadDownloader {
    ...
    if (registry.squads && registry.squads.official) {
      for (const squad of registry.squads.official) {
        squads.push({
          ...
          type: 'official',
          author: squad.author || 'Trivia-Growth',
        });
      }
    }
  ...
  /**
   * Fetch registry from triviaiox-squads repository
   *
  ...
  async _downloadSquadFiles(squadInfo, targetPath) {
    ...
    if (!Array.isArray(contents)) {
      throw new SquadDownloaderError(
        ...
        'Check if squad exists in triviaiox-squads repository',
      );
    }
  ...
  _fetch(url, useApi = false) {
    return new Promise((resolve, reject) => {
      const options = {
        headers: {
          'User-Agent': 'TRIVIAIOX-SquadDownloader/1.0',
        },
      };
