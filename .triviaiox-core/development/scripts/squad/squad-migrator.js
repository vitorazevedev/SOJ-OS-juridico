...
class SquadMigrator {
  ...
  async analyze(squadPath) {
    ...
    try {
      ...
      // Check for missing triviaiox.type
      if (!manifest.triviaiox?.type) {
        analysis.needsMigration = true;
        analysis.issues.push({
          type: 'MISSING_TRIVIAIOX_TYPE',
          message: 'Missing required field: triviaiox.type',
          severity: 'error',
        });
        analysis.actions.push({
          type: 'ADD_FIELD',
      ...
      // Check for missing triviaiox.minVersion
      if (!manifest.triviaiox?.minVersion) {
        analysis.needsMigration = true;
        analysis.issues.push({
          type: 'MISSING_MIN_VERSION',
          message: 'Missing required field: triviaiox.minVersion',
          severity: 'error',
        });
        analysis.actions.push({
          type: 'ADD_FIELD',
          path: 'triviaiox.minVersion',
          value: '2.1.0',
        });
      }
