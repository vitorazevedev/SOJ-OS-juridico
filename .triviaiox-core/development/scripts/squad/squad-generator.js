...
/**
 * Default TRIVIAIOX minimum version
 * @constant {string}
 */
const DEFAULT_TRIVIAIOX_MIN_VERSION = '2.1.0';
...
class SquadGeneratorError extends Error {
  ...
  static schemaNotFound(schemaPath) {
    return new SquadGeneratorError(
      ...
      'Ensure TRIVIAIOX is properly installed',
    );
  }
}
...
triviaiox:
  minVersion: "${config.triviaioxMinVersion || DEFAULT_TRIVIAIOX_MIN_VERSION}"
  type: squad
...
${config.description || 'Custom TRIVIAIOX squad.'}
...
This squad ${config.configMode === 'extend' ? 'extends' : config.configMode === 'override' ? 'overrides' : 'does not inherit'} the core TRIVIAIOX configuration.
...
> This file ${config.configMode === 'extend' ? 'extends' : config.configMode === 'override' ? 'overrides' : 'is independent of'} the core TRIVIAIOX coding standards.
...
- Node.js >= 18.x
- TRIVIAIOX >= ${config.triviaioxMinVersion || DEFAULT_TRIVIAIOX_MIN_VERSION}
...
class SquadGenerator {
   ...
   * @param {string} [config.triviaioxMinVersion] - Minimum TRIVIAIOX version
  ...
  async generate(config) {
    ...
    const fullConfig = {
      ...
      triviaioxMinVersion: config.triviaioxMinVersion || DEFAULT_TRIVIAIOX_MIN_VERSION,
