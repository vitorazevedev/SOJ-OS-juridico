/**
 * TRIVIAIOX Skill Validator
 * Story GEMINI-INT.5 - Skills Cross-CLI Compatibility
 *
 * Validates that TRIVIAIOX agent skills work correctly in both
...
class SkillValidator {
  constructor(config = {}) {
    this.agentsDir = config.agentsDir || path.join(process.cwd(), '.triviaiox-core', 'development', 'agents');
    this.tasksDir = config.tasksDir || path.join(process.cwd(), '.triviaiox-core', 'development', 'tasks');
    this.errors = [];
    this.warnings = [];
  }
  ...
  validateGeminiCompatibility(agentDef, result) {
    // Gemini CLI uses .gemini/rules/TRIVIAIOX/agents/ structure
  ...
  generateReport(results) {
    let report = '# TRIVIAIOX Skills Cross-CLI Compatibility Report\n\n';
