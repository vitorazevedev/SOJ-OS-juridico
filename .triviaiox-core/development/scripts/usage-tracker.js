...
/**
 * Usage tracker for TRIVIAIOX-FULLSTACK framework components
 * Tracks component usage patterns for deprecation warnings and impact analysis
 */
class UsageTracker {
  constructor(options = {}) {
    this.rootPath = options.rootPath || process.cwd();
    this.usageDir = path.join(this.rootPath, '.triviaiox', 'usage');
  ...
  async getComponentInfo(componentId) {
    ...
    return {
      ...
      type: type,
      name: name,
      file_path: `triviaiox-core/${type}s/${name}.md`
    };
  }
