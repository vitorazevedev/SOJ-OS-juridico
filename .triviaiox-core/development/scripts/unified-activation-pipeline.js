 ...
 * Can be overridden via core-config.yaml pipeline.timeout_ms or TRIVIAIOX_PIPELINE_TIMEOUT env var.
 * @type {number}
 */
...
const ALL_AGENT_IDS = [
  ...
  'devops', 'triviaiox-master', 'squad-creator',
];
...
class UnifiedActivationPipeline {
   ...
   * Priority: TRIVIAIOX_PIPELINE_TIMEOUT env > core-config.yaml pipeline.timeout_ms > default
   * @private
  ...
  _resolvePipelineTimeout(coreConfig) {
    ...
    const envTimeout = process.env.TRIVIAIOX_PIPELINE_TIMEOUT;
    if (envTimeout) {
