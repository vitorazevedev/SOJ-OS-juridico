...
- **Tool:** validation-engine
  ...
  - **Source:** .triviaiox-core/utils/validation-engine.js
...
- **Script:** run-validation.js
  ...
  - **Language:** JavaScript
  - **Location:** .triviaiox-core/scripts/run-validation.js
...
# Review Proposal - TRIVIAIOX Developer Task
...
class ReviewProposalTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('📋 TRIVIAIOX Proposal Review'));
  ...
  async loadProposal(proposalId) {
    try {
      const proposalFile = path.join(this.rootPath, '.triviaiox', 'proposals', `${proposalId}.json`);
  ...
  async checkForConflicts(proposal) {
    ...
    const indexFile = path.join(this.rootPath, '.triviaiox', 'proposals', 'index.json');
  ...
  async getReviewDetails(proposal, analysis, config) {
    ...
    // Set reviewer info
    details.reviewer = process.env.USER || 'triviaiox-reviewer';
  ...
  async storeReview(proposal, review) {
    const reviewsDir = path.join(this.rootPath, '.triviaiox', 'proposals', 'reviews');
  ...
  async updateProposalStatus(proposal, review) {
    ...
    const proposalFile = path.join(this.rootPath, '.triviaiox', 'proposals', `${proposal.proposalId}.json`);
