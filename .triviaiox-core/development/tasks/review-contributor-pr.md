...
# CI/Workflow = .github/
# Test = tests/
# Code = packages/, .triviaiox-core/, bin/
...
#### For Code PRs (packages/, .triviaiox-core/, bin/)
...
gh pr diff {pr_number} -- 'packages/' '.triviaiox-core/' 'bin/' | grep -E "(shell:\s*true|execSync\(|\.exec\(|eval\(|Function\()"
