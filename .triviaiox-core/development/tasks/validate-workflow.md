...
- **Script:** workflow-validator.js
  ...
  - **Language:** JavaScript
  - **Location:** .triviaiox-core/development/scripts/workflow-validator.js
...
To validate workflow YAML files against TRIVIAIOX conventions, checking structure, agent references, artifact flow, and logical consistency. Supports validating a single workflow or all workflows in a given context (core or squad).

## Prerequisites

- WorkflowValidator class available at `.triviaiox-core/development/scripts/workflow-validator.js`
...
- Scan directory based on target_context:
  - `core` → all `.yaml` files in `.triviaiox-core/development/workflows/`
...
- Called by `*validate-workflow` command in triviaiox-master
