...
outputs:
  - name: patterns_markdown
    type: string
    destino: File (.triviaiox/patterns.md)
    persistido: true
...
pre-conditions:
  - [ ] Project root is a valid codebase
    tipo: pre-condition
    blocker: true
    validação: Check package.json or .triviaiox-core exists
...
```javascript
const PatternExtractor = require('.triviaiox-core/infrastructure/scripts/pattern-extractor');
...
Subcommands:
  ...
  save        Save to .triviaiox/patterns.md
...
Examples:
  ...
  *extract-patterns save                      # Save to .triviaiox/patterns.md
...
Output File:
  Default location: .triviaiox/patterns.md
...
Patterns saved to: .triviaiox/patterns.md
...
post-conditions:
  - [ ] Patterns file created/updated
    tipo: post-condition
    blocker: false
    validação: Check .triviaiox/patterns.md exists and is valid markdown
...
node .triviaiox-core/infrastructure/scripts/pattern-extractor.js [command] [options]

# Via TRIVIAIOX command
...
dependencies:
  modules:
    - .triviaiox-core/infrastructure/scripts/pattern-extractor.js
  tasks: []
  referenced_by:
    - spec-write.md
tags:
