...
const configPath = path.join(__dirname, '../../.triviaiox-core/core-config.yaml');
...
mkdir -p .backups
cp -r .triviaiox-core/utils ".backups/utils.backup-3.18-$(date +%Y%m%d-%H%M%S)"
```
...
grep -r "require.*utility-name" .triviaiox-core/agents .triviaiox-core/tasks .triviaiox-core/workflows Squads/

# Check for string references (utility mentioned in docs/configs)
grep -r "utility-name" .triviaiox-core/agents/ .triviaiox-core/tasks/ .triviaiox-core/core-config.yaml

# Count total references
count=$(grep -r "utility-name" .triviaiox-core/ Squads/ 2>/dev/null | wc -l)
...
```bash
mkdir -p .triviaiox-core/utils-archive
```

**3.2 Create Archive README**

Create `.triviaiox-core/utils-archive/ARCHIVE-README.md`:
...
1. **Copy file back**:
   ```bash
   cp .triviaiox-core/utils-archive/utility-name.js .triviaiox-core/scripts/
   ```
...
4. **Test thoroughly**:
   ```bash
   node .triviaiox-core/scripts/test-utilities.js
   ```
...
**Immediate Rollback**:
```bash
rm -rf .triviaiox-core/utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS .triviaiox-core/utils
```

**Selective Restoration**:
```bash
cp .triviaiox-core/utils-archive/specific-utility.js .triviaiox-core/scripts/
```
...
- **Tool:** task-runner
  ...
  - **Source:** .triviaiox-core/core/task-runner.js

- **Tool:** logger
  - **Purpose:** Execution logging and error tracking
  - **Source:** .triviaiox-core/utils/logger.js
...
- **Script:** execute-task.js
  ...
  - **Language:** JavaScript
  - **Location:** .triviaiox-core/scripts/execute-task.js
...
git mv .triviaiox-core/scripts/utility-name.js .triviaiox-core/utils-archive/

# Or for multiple files:
git mv .triviaiox-core/scripts/{triviaiox-validator-fixed.js,triviaiox-validator-refactored.js} .triviaiox-core/utils-archive/
```
...
git mv .triviaiox-core/scripts/triviaiox-validator.test.js tests/utils/
```
...
1. triviaiox-validator-fixed.js - Duplicate of triviaiox-validator.js
2. triviaiox-validator-refactored.js - Duplicate of triviaiox-validator.js
...
...
Update utility count in `.triviaiox-core/core-config.yaml`:
...
framework:
  entities:
    utils:
      ...
      location: .triviaiox-core/scripts/
```
...
```bash
node .triviaiox-core/scripts/triviaiox-validator.js
```
...
for util in $(ls .triviaiox-core/utils-archive/*.js); do
  name=$(basename $util .js)
  refs=$(grep -r "require.*$name" .triviaiox-core/agents .triviaiox-core/tasks 2>/dev/null | wc -l)
  if [ $refs -gt 0 ]; then
...
rm -rf .triviaiox-core/utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS .triviaiox-core/utils
git checkout .triviaiox-core/core-config.yaml
```
...
1. `.triviaiox-core/utils-archive/` - Archive directory with 30 deprecated utilities
2. `.triviaiox-core/utils-archive/ARCHIVE-README.md` - Archive documentation
3. `.backups/utils.backup-3.18-YYYYMMDD-HHMMSS/` - Timestamped backup
4. Updated `.triviaiox-core/core-config.yaml` - Corrected utility count
...
- ✅ Framework validation (triviaiox-validator.js) passes
- ✅ All agents (@dev, @po, @qa) activate successfully
