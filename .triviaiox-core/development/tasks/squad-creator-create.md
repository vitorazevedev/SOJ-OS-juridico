...
Cria um novo squad seguindo a arquitetura task-first do TRIVIAIOX.
...
? Config inheritance:
  > extend (adiciona as regras do core)
    ...
    none (sem heranca)
? Minimum TRIVIAIOX version: (2.1.0)
```
...
📚 Documentation:
   ...
   - Task Format: .triviaiox-core/docs/standards/TASK-FORMAT-SPECIFICATION-V1.md

🚀 When ready to share:
   - Local only: Keep in ./squads/ (private)
   - Public: @squad-creator *publish-squad meu-dominio-squad
   - API: @squad-creator *sync-squad-triviagrowth meu-dominio-squad
```
...
```javascript
const { SquadGenerator } = require('./.triviaiox-core/development/scripts/squad');
