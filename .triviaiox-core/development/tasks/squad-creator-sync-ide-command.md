...
Checklist:
  - '[x] Carregar .triviaiox-sync.yaml'
...
O sistema usa `.triviaiox-sync.yaml` na raiz do projeto para configuração:
...
| `Squad alias not found` | Squad não está em squad_aliases | Adicione ao .triviaiox-sync.yaml |
| `File exists`          | Destino já existe               | Use --force ou escolha ação |
| `IDE not active`       | IDE não está em active_ides     | Ative no .triviaiox-sync.yaml    |
...
const syncConfig = loadYaml('.triviaiox-sync.yaml');
...
- `*sync-squad-triviagrowth` - Sincronizar para TriviaGrowth marketplace
