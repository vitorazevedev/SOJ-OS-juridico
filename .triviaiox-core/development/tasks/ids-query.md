...
```yaml
pre-conditions:
  - [ ] Entity Registry exists at .triviaiox-core/data/entity-registry.yaml
    tipo: pre-condition
    blocker: false
    validação: |
...
```bash
# Basic query
node bin/triviaiox-ids.js ids:query "validate story drafts"

# With JSON output
node bin/triviaiox-ids.js ids:query "template rendering engine" --json

# Filter by type
node bin/triviaiox-ids.js ids:query "database migration" --type script

# Filter by category
node bin/triviaiox-ids.js ids:query "agent persona" --category agents
```
...
const { RegistryLoader } = require(path.resolve('.triviaiox-core/core/ids/registry-loader'));
const { IncrementalDecisionEngine } = require(path.resolve('.triviaiox-core/core/ids/incremental-decision-engine'));
...
- `triviaiox ids:create-review` — Review CREATE decisions for 30-day assessment
