...
Blocks are small, focused units of functionality that can be included in multiple tasks. They follow the DRY (Don't Repeat Yourself) principle and provide consistent behavior across the TRIVIAIOX framework.

## Directory Structure

```
.triviaiox-core/development/tasks/blocks/
├── README.md                  # This file
...
```javascript
const { loadBlock, executeBlock } = require('.triviaiox-core/utils/block-loader');
...
| `agent-prompt-template` | Standardized template for spawning TRIVIAIOX agents | Any skill/task that invokes agents via Task tool |
...
*TRIVIAIOX Task Blocks System v1.1.0*
