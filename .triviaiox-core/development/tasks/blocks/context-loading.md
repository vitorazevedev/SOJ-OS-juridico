...
Load TRIVIAIOX project context before task execution. Provides git state, gotchas filtered by category, technical preferences, and core configuration.
...
steps:
  ...
  - name: Load Technical Preferences
    ...
    actions:
      - read: .triviaiox-core/data/technical-preferences.md
    output: preferences

  - name: Load Core Config
    actions:
      - read: .triviaiox-core/core-config.yaml
...
```javascript
const { loadContext } = require('.triviaiox-core/utils/context-loader');
...
| File | Purpose |
|------|---------|
| `.triviaiox/gotchas.json` | Known issues and workarounds |
| `.triviaiox-core/data/technical-preferences.md` | User-defined patterns |
| `.triviaiox-core/core-config.yaml` | Project configuration |
