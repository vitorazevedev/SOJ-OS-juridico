...
Standardized template for spawning TRIVIAIOX agents with consistent structure. Ensures all agent invocations follow the same persona loading, context, mission, and output pattern.
...
```markdown
You are {agent_name}, the TRIVIAIOX {agent_role}. Read your complete agent file at:
{agent_file_path}
...
<!-- Parameters:
  ...
  agent_file_path=.claude/commands/TRIVIAIOX/agents/architect.md,
...
```javascript
const { loadBlock, renderTemplate } = require('.triviaiox-core/utils/block-loader');
