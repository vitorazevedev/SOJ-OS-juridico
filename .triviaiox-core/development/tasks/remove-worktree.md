...
post-conditions:
  - [ ] Worktree directory removed
    tipo: post-condition
    blocker: true
    validação: Directory .triviaiox/worktrees/{storyId} does not exist
...
Removes an TRIVIAIOX-managed worktree and its associated branch. Includes safety checks for uncommitted changes and provides options for force removal.
...
Options:
  1. Commit changes first    : cd .triviaiox/worktrees/{storyId} && git commit
...
You are about to remove:
  • Worktree: .triviaiox/worktrees/{storyId}
  • Branch:   auto-claude/{storyId}
...
1. `git worktree remove .triviaiox/worktrees/{storyId}`
...
❌ Cannot remove worktree while inside it.
   ...
   Current directory: .triviaiox/worktrees/{storyId}
...
```bash
# Remove worktree
git worktree remove .triviaiox/worktrees/{storyId} --force
...
- `.triviaiox-core/infrastructure/scripts/worktree-manager.js`
