...
autoClaude:
  ...
  inputs:
    ...
    - name: projectContext
      type: file
      path: .triviaiox/project-context.yaml
      required: false
...
step_11:
  ...
  actions:
    ...
    - action: update_dashboard_status
      script: |
        const { PlanTracker } = require('.triviaiox-core/infrastructure/scripts/plan-tracker.js');
...
recovery:
  ...
  attempt_tracking:
    storage: .triviaiox/recovery/{storyId}/{subtaskId}.json
    schema:
      subtaskId: string
      attempts: array
