...
- campo: gotchas_file
  tipo: string
  destino: .triviaiox/gotchas.md
  persistido: true

- campo: gotchas_json
  tipo: string
  destino: .triviaiox/gotchas.json
  persistido: true
...
```yaml
pre-conditions:
  - [ ] Project has .triviaiox directory initialized
    tipo: pre-condition
    blocker: false
    validação: |
      Check if .triviaiox/ directory exists, create if not
    error_message: "Creating .triviaiox/ directory"
...
post-conditions:
  - [ ] gotchas.md file generated/updated
    ...
    blocker: true
    validação: |
      Check .triviaiox/gotchas.md exists and has content
  ...
  - [ ] gotchas.json schema valid
    ...
    blocker: false
    validação: |
      Validate .triviaiox/gotchas.json against schema
...
1. **update** (default)
   ...
   - Generates `.triviaiox/gotchas.md` and `.triviaiox/gotchas.json`
...
```javascript
const { GotchasDocumenter } = require('.triviaiox-core/infrastructure/scripts/gotchas-documenter');

const documenter = new GotchasDocumenter(rootPath, {
  outputPath: '.triviaiox/gotchas.md',
  quiet: false
});
```
...
documenter.mergeWithExisting('.triviaiox/gotchas.json');
```
...
afterCapture: async (insightsPath) => {
  const { updateGotchas } = require('.triviaiox-core/infrastructure/scripts/gotchas-documenter');
...
const { getGotchasForSelfCritique } = require('.triviaiox-core/infrastructure/scripts/gotchas-documenter');
...
node .triviaiox-core/infrastructure/scripts/gotchas-documenter.js update

# List all gotchas
node .triviaiox-core/infrastructure/scripts/gotchas-documenter.js list

# List high severity only
node .triviaiox-core/infrastructure/scripts/gotchas-documenter.js list --severity high

# Search for specific gotchas
node .triviaiox-core/infrastructure/scripts/gotchas-documenter.js search "zustand"

# List by category
node .triviaiox-core/infrastructure/scripts/gotchas-documenter.js category TypeScript

# Output as JSON
node .triviaiox-core/infrastructure/scripts/gotchas-documenter.js list --format json
```
...
3. **Error:** Write permission denied
   - **Cause:** Cannot write to .triviaiox directory
...
**Script:** gotchas-documenter.js
  ...
  - **Language:** JavaScript
  - **Location:** .triviaiox-core/infrastructure/scripts/gotchas-documenter.js

---

## Dependencies

- `.triviaiox-core/development/tasks/capture-session-insights.md` - Provides input insights
- `.triviaiox-core/product/checklists/self-critique-checklist.md` - Consumes gotchas (AC5)
- `.triviaiox-core/development/tasks/spec-write-spec.md` - May reference gotchas
