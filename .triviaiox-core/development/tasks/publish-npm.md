...
5. PUBLISH
   ...
   - Verify: npm view triviaiox-core@preview version === {new_version}

6. SMOKE TEST
   - Create temp directory
   - npm init -y
   - npm install triviaiox-core@preview
   - Verify critical files exist in node_modules/triviaiox-core/:
...
8. REPORT
   ...
   - "Test with: npm install triviaiox-core@preview"
...
- [ ] `npm view triviaiox-core@preview` returns new version
...
1. VERIFY PREVIEW EXISTS
   - npm view triviaiox-core@preview version
   - If no preview: HALT with "No preview version found"

2. CONFIRM WITH USER
   - Display: "Promote v{X.Y.Z} from preview to latest?"
   - Show current latest: npm view triviaiox-core@latest version
   - Require explicit confirmation

3. PROMOTE
   - npm dist-tag add triviaiox-core@{version} latest

4. VERIFY
   - npm view triviaiox-core@latest version === {version}
...
6. REPORT
   ...
   - Release URL
   - "Install with: npm install triviaiox-core"
```

#### Pre-conditions

- [ ] Preview version exists (`npm view triviaiox-core@preview`)
...
- [ ] `npm view triviaiox-core@latest` returns promoted version
...
2. INSTALL
   - npm install triviaiox-core@{tag} (default: latest)
   - Record: install time, exit code, warnings

3. VERIFY FILES
   - Check node_modules/triviaiox-core/ contains:
     - .triviaiox-core/core-config.yaml
     - .triviaiox-core/constitution.md
     - .triviaiox-core/development/agents/ (non-empty)
     - .triviaiox-core/development/tasks/ (non-empty)
     - .claude/hooks/synapse-engine.cjs
     - .claude/hooks/precompact-session-digest.cjs
     - .claude/rules/ (non-empty)
     - bin/triviaiox.js
     - bin/triviaiox-minimal.js
   - Check node_modules/triviaiox-core/ does NOT contain:
     - pro/
     - .env
     - .git/
     - tests/
...
4. TEST INSTALLER (optional, if --full flag)
   - npx triviaiox-core install --preset minimal
   - Verify: .triviaiox-core/ created
...
- `--full`: Also test `npx triviaiox-core install`
...
npm view triviaiox-core versions --json

# 2. Point latest back to previous version
npm dist-tag add triviaiox-core@{previous-good-version} latest

# 3. Deprecate broken version with message
npm deprecate triviaiox-core@{broken-version} "Known issues, use v{previous-good-version}"
...
To fully block a version, use `npm unpublish triviaiox-core@{version}` (within 72h of publish only).
