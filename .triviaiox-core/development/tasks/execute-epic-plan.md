...
- campo: epic_state
  tipo: object
  destino: File system (.triviaiox/epic-{epicId}-state.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] Template workflow must exist
    ...
    blocker: true
    validação: |
      execution.template must resolve to .triviaiox-core/development/workflows/{template}.yaml
  ...
  - [ ] For action=continue, state file must exist
    ...
    blocker: true
    validação: |
      .triviaiox/epic-{epicId}-state.yaml must exist with status=active
...
post-conditions:
  - [ ] State file updated with current wave progress
    ...
    blocker: true
    validação: |
      .triviaiox/epic-{epicId}-state.yaml exists and reflects completed waves/stories
...
4. **Error:** State file corrupted
   ...
   - **Recovery:** Restore from .triviaiox/epic-{epicId}-state.yaml.bak
...
- Template `epic-orchestration.yaml` exists in `.triviaiox-core/development/workflows/`
- Inner loop `development-cycle.yaml` exists in `.triviaiox-core/development/workflows/`
...
```yaml
# .triviaiox/epic-{epicId}-state.yaml
epic_state:
  epicId: {epicId}
...
**1. Load state** from `.triviaiox/epic-{epicId}-state.yaml`
...
State preserved at: .triviaiox/epic-{epicId}-state.yaml
...
PROCEDURE execute_wave(wave, stories, state):
  ...
  Task tool call:
    ...
    subagent_type: "triviaiox-architect"  # or whatever the gate agent is
    prompt: |
  ...
  IF gate verdict == APPROVED:
    ...
    Task tool call:
      ...
      subagent_type: "triviaiox-devops"
      prompt: |
...
PROCEDURE final_gate(execution, state):
  ...
  Task tool call:
    ...
    subagent_type: "triviaiox-architect"
    prompt: |
