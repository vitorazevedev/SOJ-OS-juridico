# Task: Update TRIVIAIOX Framework

> **Version:** 4.0.0
> **Created:** 2026-01-29
> **Updated:** 2026-01-31
> **Type:** SYNC (git-native framework synchronization)
> **Agent:** @devops (Gage) or @triviaiox (Orion)
> **Execution:** Simple bash script (~15 lines)

## Purpose

Git-native sync of TRIVIAIOX framework from upstream repository. Uses sparse clone + file comparison for safe review before applying changes. All local customizations preserved automatically by backup/restore.
...
bash .triviaiox-core/scripts/update-triviaiox.sh

# Review changes shown by the script, then:
git add .triviaiox-core && git commit -m "chore: sync TRIVIAIOX framework"   # Apply changes
# OR
git checkout -- .triviaiox-core/                                         # Cancel changes
```
...
1. **Clone upstream** - Sparse shallow clone of Trivia-Growth/Triviaiox (only `.triviaiox-core/`)
...
| Path | Reason |
|------|--------|
| `.triviaiox-core/squads/` | Custom copywriters, data, ralph |
| `.triviaiox-core/marketing/` | Marketing-specific agents/tasks |
| `source/` | Business context YAML |
| `Knowledge/` | Knowledge bases |
| `.triviaiox-core/context/` | Compiled contexts |
...
```yaml
task: updateTRIVIAIOXFramework
agent: devops
mode: simple
timeout: 60  # 1 minute max

execution:
  script: .triviaiox-core/scripts/update-triviaiox.sh

workflow:
  1. If dirty working tree: git add -A && git commit -m "chore: pre-update commit"
  2. bash .triviaiox-core/scripts/update-triviaiox.sh
  3. Review changes displayed
  4. git add .triviaiox-core && git commit -m "chore: sync TRIVIAIOX framework"  # to apply
  5. git checkout -- .triviaiox-core/                                        # to cancel
...
ls -la .triviaiox-core/squads/  # if exists
