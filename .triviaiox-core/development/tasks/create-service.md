...
outputs:
  - name: service_directory
    type: directory
    location: ".triviaiox-core/infrastructure/services/{service_name}/"
    persistido: true
...
```yaml
pre-conditions:
  - [ ] WIS-10 templates exist at .triviaiox-core/development/templates/service-template/
    tipo: pre-condition
    blocker: true
  ...
  - [ ] Service name is unique (no existing service with same name)
    tipo: pre-condition
    blocker: true
    validação: Check .triviaiox-core/infrastructure/services/{name}/ does not exist
...
3. agent-tool - Tool for TRIVIAIOX agents
...
const { checkBeforeWriting } = require('.triviaiox-core/core/code-intel/helpers/dev-helper');
...
// Check uniqueness
const targetDir = `.triviaiox-core/infrastructure/services/${serviceName}/`;
...
```javascript
const templateDir = '.triviaiox-core/development/templates/service-template/';
const templates = [
  'README.md.hbs',
  'index.ts.hbs',
  'types.ts.hbs',
...
cd .triviaiox-core/infrastructure/services/{service_name}/
...
============================================
 ...
 Next Steps:
   1. cd .triviaiox-core/infrastructure/services/{service_name}
