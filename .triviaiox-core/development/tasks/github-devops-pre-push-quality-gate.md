...
- Repository context detected (run `triviaiox init` if needed)
...
if (!context) {
  ...
  console.error('Run "triviaiox init" to configure installation mode');
  process.exit(1);
}
...
```javascript
const { assessPrePushImpact, classifyRiskLevel } = require('.triviaiox-core/core/code-intel/helpers/devops-helper');
