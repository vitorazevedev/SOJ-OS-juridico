...
- PM tool configured in `.triviaiox-pm-config.yaml` (or will use local-only mode)
...
```javascript
const { getPMAdapter } = require('../.triviaiox-core/scripts/pm-adapter-factory');
...
- **Configuration missing**: Inform user to run `triviaiox init`
...
```javascript
const { syncStoryToPM } = require('../.triviaiox-core/scripts/story-manager');
