...
To help users contribute to the TRIVIAIOX open-source project (`triviaiox-core`) by automating the PR creation process, ensuring contributions follow project standards, pass quality checks, and have proper formatting before submission.

**Target Repository**: `triviaiox-core` (open-source framework)
...
- **contribution_path**: `string`
  ...
  - **Example**: `"Squads/my-new-pack/"` or `"triviaiox-core/agents/improved-agent.md"`
...
- **pr_url**: `string`
  ...
  - **Example**: `"https://github.com/Trivia-Growth/Triviaiox/pull/123"`
...
2. **Validate Repository State**
   - Check if `triviaiox-core` repository is set as upstream
...
3. **Detect Contribution Type** (if not provided)
   - Scan modified files:
     - `Squads/*` → "Squad"
     - `triviaiox-core/agents/*` → "agent"
     - `triviaiox-core/tasks/*` → "task"
     - `triviaiox-core/tools/*` → "tool"
...
14. **Generate PR Title & Description**
    ...
    - **Description**: Use PR template:
      ...
      **First-time contributor?** Welcome! 🎉 This PR was created using TRIVIAIOX PR Automation.
      ```
...
18. **Provide Next Steps**
    - Display to user:
      ...
      Thank you for contributing to TRIVIAIOX! 🚀
      ```
...
- [ ] Fork of triviaiox-core exists
  - **Validation**: `gh repo view {user}/triviaiox-core` succeeds
...
```bash
triviaiox activate Otto  # github-devops agent
triviaiox pr create \
  --type="Squad" \
...
triviaiox pr create \
  --type="agent" \
  --path="triviaiox-core/agents/improved-po.md" \
...
triviaiox pr create \
  --type="bug-fix" \
  --path="triviaiox-core/tasks/create-next-story.md" \
