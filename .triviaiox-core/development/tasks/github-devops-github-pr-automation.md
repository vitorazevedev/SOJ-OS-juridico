...
if (!context) {
  throw new Error('Unable to detect repository. Run "triviaiox init" first.');
}
```
...
function loadPRConfig() {
  const configPath = path.join(process.cwd(), '.triviaiox-core', 'core-config.yaml');
...
```javascript
const { generateImpactSummary } = require('.triviaiox-core/core/code-intel/helpers/devops-helper');
...
### NPM Package with Semantic-Release (triviaiox-core)
```yaml
github:
  pr:
