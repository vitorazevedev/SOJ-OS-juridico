...
- **Tool:** code-analyzer
  ...
  - **Source:** .triviaiox-core/utils/code-analyzer.js
...
- **Script:** analyze-codebase.js
  ...
  - **Language:** JavaScript
  - **Location:** .triviaiox-core/scripts/analyze-codebase.js
...
Systematically audit all utilities in `.triviaiox-core/scripts/` to determine their functional status, classify them as WORKING/FIXABLE/DEPRECATED, and generate actionable recommendations for maintenance and cleanup.
...
for util in .triviaiox-core/scripts/*.js; do
  name=$(basename $util .js)
  count=$(grep -r "$name" .triviaiox-core/agents .triviaiox-core/tasks Squads 2>/dev/null | wc -l)
