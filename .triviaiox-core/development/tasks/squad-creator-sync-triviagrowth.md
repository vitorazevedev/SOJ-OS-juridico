---
task: Sync Squad to TriviaGrowth
...
Entrada: |
  ...
  - official: Flag para marcar como oficial (--official, apenas Trivia-Growth)
...
Checklist:
  ...
  - "[x] Calcular checksum"
  - "[x] Enviar para TriviaGrowth API"
  - "[x] Exibir URL do marketplace"
---

# *sync-squad-triviagrowth

Sincroniza um squad local para o TriviaGrowth API marketplace.
...
*sync-squad-triviagrowth ./squads/meu-squad

# Sync público (visível para todos)
*sync-squad-triviagrowth ./squads/meu-squad --public

# Preview sem sincronizar
*sync-squad-triviagrowth ./squads/meu-squad --dry-run

# Sync com verbosidade
*sync-squad-triviagrowth ./squads/meu-squad --verbose
```

## Autenticação

Requer autenticação com TriviaGrowth API:

```bash
export TRIVIAGROWTH_API_TOKEN="seu-token"
```

Ou configure em `.env`:

```env
TRIVIAGROWTH_API_URL=https://api.triviagrowth.dev/api
TRIVIAGROWTH_API_TOKEN=seu-token
```

Para obter um token:
1. Acesse https://triviagrowth.dev/settings/api-keys
...
Step 3: Sync to TriviaGrowth API
  ✓ Visibility: private
  ✓ API URL: https://api.triviagrowth.dev/api

Syncing to TriviaGrowth API...
...
Next steps:
  ...
  - Make public: *sync-squad-triviagrowth ./squads/meu-squad --public
```
...
| `--official` | Marca como squad oficial (apenas Trivia-Growth team) | false |
...
│     ↓                                             │
│  5. Check TRIVIAGROWTH_API_TOKEN                        │
│     ↓ (missing → abort)                           │
...
```javascript
POST ${TRIVIAGROWTH_API_URL}/squads/sync
Authorization: Bearer ${TRIVIAGROWTH_API_TOKEN}
...
// 3. Validate with squad-validator.js
const { validateSquad } = await import('.triviaiox-core/development/scripts/squad/squad-validator.js');
...
const apiToken = process.env.TRIVIAGROWTH_API_TOKEN;
const apiUrl = process.env.TRIVIAGROWTH_API_URL || 'https://api.triviagrowth.dev/api';

if (!apiToken) {
  error('TRIVIAGROWTH_API_TOKEN not set. See task docs for authentication.');
  return;
}
...
✅ Squad synced successfully!
  ...
  if (flags.public) {
    output(`  URL: https://triviagrowth.dev/squads/${result.data.squad_id}`);
  }
} else {
...
| `TRIVIAGROWTH_API_TOKEN not set` | Token não configurado | Configure a variável de ambiente |
| `401 Unauthorized` | Token inválido ou expirado | Gere novo token em triviagrowth.dev |
...
- **SQS-5:** SquadSyncService for TriviaGrowth API (Sprint 8)
