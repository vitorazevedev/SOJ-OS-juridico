...
} = require('./.triviaiox-core/scripts/decision-recorder');
```
...
2. **Code Intelligence Check (IDS G4)** — Before creating new files or functions:
   - If code intelligence is available (`isCodeIntelAvailable()` from `.triviaiox-core/core/code-intel`):
     - Call `checkBeforeWriting(fileName, description)` from `.triviaiox-core/core/code-intel/helpers/dev-helper`
...
7. Execute `.triviaiox-core/product/checklists/story-dod-checklist.md`
...
│    │    wsl bash -c 'cd /mnt/c/.../triviaiox-core &&    │   │
...
- `.triviaiox-core/product/checklists/story-dod-checklist.md` - Definition of Done checklist
