...
Configure complete GitHub DevOps infrastructure for user projects created with TRIVIAIOX. This task copies GitHub Actions workflows, configures CodeRabbit, sets up branch protection, and manages secrets.
...
- campo: devops_setup_report
  tipo: object
  destino: File system (.triviaiox/devops-setup-report.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] Not already configured (idempotency check)
    ...
    blocker: false
    validação: |
      Check .triviaiox/devops-setup-report.yaml existence
...
post-conditions:
  ...
  - [ ] DevOps setup report generated
    ...
    blocker: false
    validação: |
      Test-Path ".triviaiox/devops-setup-report.yaml"
...
acceptance-criteria:
  ...
  - [ ] Setup report documents all configurations
    ...
    blocker: true
    validação: |
      .triviaiox/devops-setup-report.yaml contains all setup details
...
# Check idempotency
if (Test-Path ".triviaiox/devops-setup-report.yaml") {
...
$ciTemplate = Get-Content ".triviaiox-core/infrastructure/templates/github-workflows/ci.yml.template" -Raw
...
Copy-Item ".triviaiox-core/infrastructure/templates/github-workflows/pr-automation.yml.template" `
  -Destination ".github/workflows/pr-automation.yml"
Write-Host "✅ Installed pr-automation.yml"

# Copy release.yml
Copy-Item ".triviaiox-core/infrastructure/templates/github-workflows/release.yml.template" `
...
$coderabbitConfig = Get-Content ".triviaiox-core/infrastructure/templates/coderabbit.yaml.template" -Raw
...
New-Item -ItemType Directory -Path ".triviaiox" -Force | Out-Null

$report | Out-File -FilePath ".triviaiox/devops-setup-report.yaml" -Encoding utf8

Write-Host "✅ Setup report saved to .triviaiox/devops-setup-report.yaml"
```
...
║     git add .github/ .coderabbit.yaml .triviaiox/                              ║
║     git commit -m "chore: add DevOps configuration [Story 5.10]"          ║
║     git push                                                               ║
║                                                                            ║
║  Report saved: .triviaiox/devops-setup-report.yaml                             ║
║                                                                            ║
╚═══════════════════════════════════════════════════════════════════════════╝
