...
steps:
  ...
  2_check_dependencies:
    ...
    checks:
      ...
      - Does this change affect IDE sync? (.triviaiox-core/infrastructure/scripts/ide-sync/)
      ...
      - Does this change affect other agents? (.triviaiox-core/development/agents/)
...
steps:
  ...
  3_handle_edge_cases:
    ...
    known_pitfalls:
      - Email addresses inside strings may match rename patterns (Issue #159: security@trivia-growth/triviaiox-core.dev)
  ...
  4_regenerate_manifests:
    ...
    action: |
      ...
      This regenerates .triviaiox-core/install-manifest.yaml
    when: Any file in .triviaiox-core/ or packages/ is created, modified, or deleted

  5_run_ide_sync:
    condition: Changes affect agent definitions or IDE sync system
    action: |
      Run: node .triviaiox-core/infrastructure/scripts/ide-sync/index.js sync --verbose
      Verify all IDEs sync without errors
    when: Changes to .triviaiox-core/development/agents/ or ide-sync/
```
...
- **Pitfall:** `@trivia-growth/triviaiox-core` inside email `security@trivia-growth/triviaiox-core.dev` was corrupted
