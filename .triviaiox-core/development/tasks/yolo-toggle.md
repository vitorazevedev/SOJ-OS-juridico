...
Read the current permission mode from `.triviaiox/config.yaml` under `permissions.mode`.
...
Update `.triviaiox/config.yaml` with the new `permissions.mode` value.
The `PermissionMode._saveToConfig()` method handles this, including creating the `.triviaiox/` directory if needed.
...
```javascript
const { cycleMode } = require('./.triviaiox-core/core/permissions');
...
- If `.triviaiox/config.yaml` cannot be read, start from default `ask` mode
...
version: 1.0.0
dependencies:
  - .triviaiox-core/core/permissions/permission-mode.js
  - .triviaiox-core/core/permissions/index.js
tags:
  - permissions
  - mode-toggle
  - universal-command
