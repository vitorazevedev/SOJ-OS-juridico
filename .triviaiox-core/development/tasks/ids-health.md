...
```bash
node bin/triviaiox-ids.js ids:health
```
...
```bash
node bin/triviaiox-ids.js ids:health --fix
```
...
- Log all healing actions to `.triviaiox-core/data/registry-healing-log.jsonl`

### Step 3: JSON Output (Machine-Readable)

```bash
node bin/triviaiox-ids.js ids:health --json
node bin/triviaiox-ids.js ids:health --fix --json
```
...
```javascript
const { RegistryHealer } = require('.triviaiox-core/core/ids/registry-healer');
