...
function run() {
  ...
  check(1, 'Runtime capability detection module exists', () => {
    return fs.existsSync(path.join(PROJECT_ROOT, '.triviaiox-core', 'data', 'capability-detection.js'));
  });

  check(2, 'Detection runs at session init (module is importable)', () => {
    const mod = require(path.join(PROJECT_ROOT, '.triviaiox-core', 'data', 'capability-detection.js'));
  ...
  check(4, 'Tier 3 tools deferred via best available strategy', () => {
    const caps = JSON.parse(fs.readFileSync(path.join(PROJECT_ROOT, '.triviaiox', 'runtime-capabilities.json'), 'utf8'));
  ...
  check(7, 'Search accuracy validated (7/7 test queries pass)', () => {
    const { validate } = require(path.join(PROJECT_ROOT, '.triviaiox-core', 'data', 'tool-search-validation.js'));
    return validate();
  });
  ...
  check(9, 'Essential MCP servers defined in tool-registry.yaml', () => {
    const registry = fs.readFileSync(path.join(PROJECT_ROOT, '.triviaiox-core', 'data', 'tool-registry.yaml'), 'utf8');
  ...
  check(10, 'Non-essential servers can be re-enabled per-session', () => {
    const mod = fs.readFileSync(path.join(PROJECT_ROOT, '.triviaiox-core', 'data', 'mcp-discipline.js'), 'utf8');
