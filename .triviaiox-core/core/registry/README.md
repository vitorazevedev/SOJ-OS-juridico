# TRIVIAIOX Service Registry

The Service Registry is the central catalog of all workers, tasks, templates, scripts, and workflows in the TRIVIAIOX framework. It enables service discovery, search, and reuse across the system.
...
```javascript
const { getRegistry, loadRegistry } = require('.triviaiox-core/core/registry/registry-loader');
...
```bash
node .triviaiox-core/core/registry/build-registry.js
```

The builder scans:
- `.triviaiox-core/development/tasks/**/*.md` → task
- `.triviaiox-core/product/templates/**/*.md` → template
- `.triviaiox-core/infrastructure/scripts/**/*.js` → script
- `.triviaiox-core/product/checklists/**/*.md` → checklist
- `.triviaiox-core/development/workflows/**/*.yaml` → workflow
- `.triviaiox-core/core/data/**/*` → data
...
```bash
node .triviaiox-core/core/registry/validate-registry.js
```
...
```json
{
  ...
  "path": ".triviaiox-core/development/tasks/po-create-story.md",
...
- **Discovery CLI** (Story 2.7-2.9): `triviaiox search`, `triviaiox info`, `triviaiox list`
...
*Generated by TRIVIAIOX Framework v4.0.4*
