 ...
 *
 * Provides async API for spawning TRIVIAIOX agents in separate terminals
...
async function cleanupOldFiles(outputDir = os.tmpdir(), maxAgeMs = 3600000) {
  ...
  try {
    ...
    const triviaioxFiles = files.filter(
      (f) => f.startsWith('triviaiox-output-') || f.startsWith('triviaiox-lock-') || f.startsWith('triviaiox-context-'),
    );
