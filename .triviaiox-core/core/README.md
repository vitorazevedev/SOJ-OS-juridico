# TRIVIAIOX Core Module

> Central runtime module providing essential framework functionality for TriviaGrowth Triviaiox.
...
The Core module contains the foundational runtime components that all other TRIVIAIOX modules depend on. It provides configuration management, session handling, elicitation workflows, and essential utilities.

## Installation

The core module is automatically available within the TriviaGrowth Triviaiox framework:

```javascript
// CommonJS
const core = require('./.triviaiox-core/core');

// ES Modules
import { ConfigCache, ElicitationEngine } from './.triviaiox-core/core/index.esm.js';
```
...
│   ├── triviaiox-kb.md          # TRIVIAIOX knowledge base
...
```javascript
const { globalConfigCache } = require('./.triviaiox-core/core');
...
```javascript
const { loadAgentConfig } = require('./.triviaiox-core/core');
...
```javascript
const { loadConfigSections } = require('./.triviaiox-core/core');
...
```javascript
const { ContextDetector } = require('./.triviaiox-core/core');
...
```javascript
const { SessionContextLoader } = require('./.triviaiox-core/core');
...
```javascript
const { ElicitationEngine } = require('./.triviaiox-core/core');
...
```javascript
const { ElicitationSessionManager } = require('./.triviaiox-core/core');
...
```javascript
const { YAMLValidator, validateYAML } = require('./.triviaiox-core/core');
...
```javascript
const { PersonalizedOutputFormatter } = require('./.triviaiox-core/core');
...
*TriviaGrowth Triviaiox Core Module v2.0.0*
