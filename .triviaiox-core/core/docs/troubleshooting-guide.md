# TriviaGrowth Triviaiox Meta-Agent Troubleshooting Guide

## Overview

This guide helps diagnose and resolve common issues when using the TriviaGrowth Triviaiox meta-agent for component creation and management.
...
**Symptoms:**
```
❌ Error: Agent 'data-analyst' already exists at /triviaiox-core/agents/data-analyst.md
```
...
2. Check existing components:
   ```bash
   ls triviaiox-core/agents/
   ```

3. If overwriting is intended:
   ```bash
   # Remove existing component first
   rm triviaiox-core/agents/data-analyst.md
   *create-agent
   ```
...
3. Check working directory:
   ```bash
   pwd
   # Should be in triviaiox-core root
   ```
...
1. Create session directory:
   ```bash
   mkdir -p triviaiox-core/.sessions
   ```

2. Check permissions:
   ```bash
   chmod 755 triviaiox-core/.sessions
   ```
...
2. Check transaction directory:
   ```bash
   ls triviaiox-core/logs/transactions/
   ```
...
1. Manual cleanup:
   ```bash
   # Check failed files
   ls -la triviaiox-core/agents/file1.md
   
   # Remove manually if needed
   rm triviaiox-core/agents/file1.md
   ```
...
3. Restore from backup:
   ```bash
   # Check backups
   ls triviaiox-core/logs/transactions/txn-*/backups/
   ```
...
1. Check failure details:
   ...
   cat triviaiox-core/logs/transactions/latest.json
   ```
...
# Specific modules
DEBUG=triviaiox:template,triviaiox:elicitation *create-agent
...
```bash
# Application logs
tail -f triviaiox-core/logs/triviaiox-developer.log

# Transaction logs
ls -la triviaiox-core/logs/transactions/

# Error logs
grep ERROR triviaiox-core/logs/*.log
```
...
- Template syntax: `triviaiox-core/docs/template-syntax.md`
- Creation guide: `triviaiox-core/docs/component-creation-guide.md`
- API reference: `triviaiox-core/docs/api-reference.md`
...
1. **Backup current state**:
   ```bash
   tar -czf triviaiox-backup.tar.gz triviaiox-core/
   ```

2. **Reset to clean state**:
   ```bash
   git checkout -- triviaiox-core/
   npm run setup
   ```
