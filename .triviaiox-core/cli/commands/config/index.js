 ...
 *
 * Subcommands:
 *   triviaiox config show [--level <n>] [--app <name>] [--debug]
 *   triviaiox config diff --levels <a>,<b>
 *   triviaiox config migrate [--dry-run] [--force]
 *   triviaiox config validate [--level <n>]
...
function migrateAction(options) {
  ...
  try {
    ...
    if (originalStr === resolvedStr) {
      ...
      console.log('This may be expected due to key normalization. Run `triviaiox config diff` to inspect.');
    }
