...
Deno.serve(async (req) => {
  ...
  // Só a equipe Ponderum (is_ponderum_staff) pode chamar esta função — mesma
  // flag usada para liberar o painel "Dev" no menu, checada de novo aqui
  // porque o service role abaixo ignora RLS por completo.
  const { data: caller, error: callerErr } = await userClient
    .from('users')
    .select('is_ponderum_staff')
    .maybeSingle()
  if (callerErr || !caller?.is_ponderum_staff) {
