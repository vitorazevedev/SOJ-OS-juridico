-- Card "Custo Est. Mês" do painel Dev usava taxa fixa por documento/análise
-- (R$0,05 / R$0,45), desatualizada desde que o prompt de análise cresceu
-- (banco de âncoras, contexto shadow, gating). contract_analyses já guarda
-- tokens_input/tokens_output reais da Sonnet; agora contract_contents passa
-- a guardar o mesmo para as chamadas Haiku do parse (extração + metadados),
-- permitindo custo real de uso em vez de estimativa por contagem.

alter table public.contract_contents
...
begin
  ...
  select json_build_object(
    ...
    'stats', json_build_object(
      ...
      'parse_tokens_input_month',    (select coalesce(sum(cc.tokens_input), 0)::bigint
                                       from contract_contents cc
                                       where cc.parsed_at >= date_trunc('month', now())),
      'parse_tokens_output_month',   (select coalesce(sum(cc.tokens_output), 0)::bigint
                                       from contract_contents cc
                                       where cc.parsed_at >= date_trunc('month', now())),
      'analysis_tokens_input_month',  (select coalesce(sum(ca.tokens_input), 0)::bigint
                                        from contract_analyses ca
                                        where ca.created_at >= date_trunc('month', now())),
      'analysis_tokens_output_month', (select coalesce(sum(ca.tokens_output), 0)::bigint
                                        from contract_analyses ca
                                        where ca.created_at >= date_trunc('month', now()))
    ),
