 ...
 * the unified hook runner in triviaiox-core.
 *
 ...
 * }
 *
 * @see .triviaiox-core/hooks/unified/runners/precompact-runner.js
 ...
 * Framework-dev: PROJECT_ROOT/.triviaiox-core/hooks/unified/runners/precompact-runner.js
 * Installed:     PROJECT_ROOT/node_modules/triviaiox-core/.triviaiox-core/hooks/unified/runners/precompact-runner.js
 */
function resolveRunnerPath() {
  const fs = require('fs');
  const candidates = [
    path.join(PROJECT_ROOT, '.triviaiox-core', 'hooks', 'unified', 'runners', 'precompact-runner.js'),
    path.join(PROJECT_ROOT, 'node_modules', 'triviaiox-core', '.triviaiox-core', 'hooks', 'unified', 'runners', 'precompact-runner.js'),
  ];
...
async function main() {
  ...
  // The child receives context via TRIVIAIOX_HOOK_CONTEXT env var and calls
  ...
  try {
    ...
    const inlineScript = [
      `const ctx = JSON.parse(process.env.TRIVIAIOX_HOOK_CONTEXT || '{}');`,
    ...
    const child = spawn(process.execPath, ['-e', inlineScript], {
      detached: true,
      stdio: 'ignore',
      env: { ...process.env, TRIVIAIOX_HOOK_CONTEXT: contextJson },
    });
    child.on('error', () => {});
    child.unref();
  } catch (_) {
