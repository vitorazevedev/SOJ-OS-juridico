...
Prevent context window accumulation when switching between TRIVIAIOX agents (`@agent` commands). Each agent switch compacts the previous agent's full persona into a structured handoff artifact (~379 tokens) instead of retaining the full definition (~3-5K tokens).

## When This Applies

This protocol activates whenever:
1. A user invokes a new agent via `@agent-name` or `/TRIVIAIOX:agents:agent-name`
...
Handoff artifacts are stored at `.triviaiox/handoffs/` (runtime, gitignored). Format: `handoff-{from}-to-{to}-{timestamp}.yaml`.

## Template Reference

Full template: `.triviaiox-core/development/templates/agent-handoff-tmpl.yaml`
