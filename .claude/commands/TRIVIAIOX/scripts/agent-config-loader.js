...
async function loadAgentRequirements() {
  ...
  const requirementsPath = path.join(process.cwd(), '.triviaiox-core', 'data', 'agent-config-requirements.yaml');
...
class AgentConfigLoader {
  ...
  async loadAgentDefinition(options = {}) {
    ...
    // Load from file
    const agentPath = path.join(process.cwd(), '.triviaiox-core', 'agents', `${this.agentId}.md`);
...
if (require.main === module) {
  ...
  (async () => {
    try {
      // Load core config
      const coreConfigPath = path.join(process.cwd(), '.triviaiox-core', 'core-config.yaml');
      ...
      switch (command) {
        ...
        case 'preload':
          const agents = agentId ? [agentId] : [
            'triviaiox-master', 'dev', 'qa', 'architect', 'po', 'pm', 'sm',
