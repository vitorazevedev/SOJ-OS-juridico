...
- [ ] Exportação e exclusão de dados funcionando e testadas manualmente — `enforce-data-retention` deployada, agendada e testada com sucesso. `delete-account` deployada mas **não testada end-to-end** (recomendo testar com uma conta de teste descartável antes de confiar nisso em produção, já que a exclusão é irreversível). Exportação (`exportOrgDataJson`) não testada manualmente via UI nesta sessão
...
- `supabase/functions/delete-account/index.ts` — novo. Exclusão completa de conta (Storage recursivo + organização com cascata + `auth.users` de cada membro). **Deployado** (não testado end-to-end)
