...
- [x] Exportação e exclusão de dados funcionando e testadas — `enforce-data-retention` deployada, agendada e testada. `delete-account` testada end-to-end em 2026-06-25 com conta descartável real (contrato + 3 arquivos em `contracts/`, `generated/`, `logo/`): confirmado 100% limpo — `organizations`, `contracts`, `public.users`, `auth.users` e `storage.objects` todos zerados após a chamada. Dois bugs reais encontrados e corrigidos nesse teste (ver Completion Notes). Exportação (`exportOrgDataJson`) ainda não testada manualmente via UI
